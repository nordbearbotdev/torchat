#!/usr/bin/env python
#Author: @nordbearbot
#Coded And Developed by: @nordbearbot
...
\__   __/(  ___  )(  ____ )(  ____ \|\     /|(  ___  )\__   __/
   ...
   | |   | (___) || ) \ \__| (____/\| )   ( || )   ( |   | |   
					...
					Coded and Developed by : NORDBEARBOT
					Co-authors : @7even, @IronMaster, @Sublime.js
                    Anonymous means of communication using Tor and AES-265 bit encryption.		
...
def start_server(host,port):
	...
	if(os_type.os_type1 == 'Linux'):
		...
		print(c.w+'['+c.g+"info"+c.w+"]: "+c.b+"{} "+c.w+"has joined your channel... Hello!"+c.d).format(user_name_peer)
		...
		if(os_type.os_type1 == 'Linux'):
			try:
				...
				ex_ = raw_input("Are you sure you would like to exit ? (y/n): ")
                	        if(ex_ == 'y'):
                	                print('\n')
                	                client.send(enc_msg(str(username+' has left the chat! Goodbye !'),key_value))
                	                client.close()
                	                try:
                	                        exit(0)
                	                except:
...
def join_server():
	...
	while True:
		try:
			...
			if(ex_ == 'y'):
				print('\n')
				sock0.send(enc_msg(str(username+' has left the chat! Goodbye!'),key_value))
				sock0.close()
				try:
					exit(0)
				except:
